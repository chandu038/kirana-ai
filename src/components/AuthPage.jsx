          ...
          <div className="w-full max-w-sm space-y-4">
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground">
                {pending ? `Continue to ask: "${pending}"` : "Please sign up or sign in below."}
              </p>
            </div>
            ...
            <div className="rounded-2xl border bg-card p-4 shadow-sm sm:p-6">
              <AuthForm
                mode={mode}
                onModeChange={(m) => navigate(m === "login" ? "/login" : "/signup")}
                onSuccess={(user) => navigate(afterLoginPath(user))}
              />
            </div>
          </div>
        </div>
